...
@interface gonggaoDetailViewController ()
@property(nonatomic,strong)UIWebView *webView;
@end

@implementation gonggaoDetailViewController
...
-(void)getData
{
    ...
    [analy gonggaodetail:self.noticeId Block:^(id models, NSString *code, NSString *msg) {
        [self stopProgress];
        if([code isEqualToString:@"1"])
        {
            NSDictionary *dict=[models mutableCopy];
            [self webViewLoadHTML:[NSString stringWithFormat:@"%@",[dict objectForKey:@"name"]] Title:[NSString stringWithFormat:@"%@",[dict objectForKey:@"add_time"]] Content:[NSString stringWithFormat:@"%@",[dict objectForKey:@"contents"]]];
        }
        else
        {
